...
from airflow.operators.topreads_plugin import DataQualityOperator
from airflow.operators.topreads_plugin import LoadAnalyticsOperator
...
dag_name = 'topreads_pipeline'
dag = DAG(dag_name,
          default_args=default_args,
          description='Load and Transform data from landing zone to processed zone. Populate data from Processed zone to topreads Warehouse.',
...
jobOperator = SSHOperator(
    task_id="TopReadsETLJob",
    command='cd /home/hadoop/topreads_etl_pipeline/src;export PYSPARK_DRIVER_PYTHON=python3;export PYSPARK_PYTHON=python3;spark-submit --master yarn topreads_driver.py;',
    ssh_hook=emrsshHook,
    dag=dag)
...
warehouse_data_quality_checks = DataQualityOperator(
    ...
    tables = ["topreads_warehouse.authors", "topreads_warehouse.reviews", "topreads_warehouse.books", "topreads_warehouse.users"]
...
authors_data_quality_checks = DataQualityOperator(
    ...
    dag=dag,
    redshift_conn_id = "redshift",
    tables = ["topreads_analytics.popular_authors_average_rating", "topreads_analytics.popular_authors_average_rating"]
...
books_data_quality_checks = DataQualityOperator(
    ...
    dag=dag,
    redshift_conn_id = "redshift",
    tables = ["topreads_analytics.popular_books_average_rating", "topreads_analytics.popular_books_review_count"]
